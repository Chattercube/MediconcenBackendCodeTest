...
- Node.js 22.13 or later in the Node.js 22 line and npm.
- Docker with the Compose plugin for the recommended database, cache and containerized API setup.
- Free host ports matching `.env`; the template uses API `3000`, MySQL `3307` and Redis `6379`.
...
Both identifiers are required, must be strings between 1 and 255 characters containing at least one non-whitespace character, and the body may not contain unknown fields. Invalid input returns HTTP 400 with field-specific messages. Unexpected application or database failures return NestJS's generic HTTP 500 response without database details.
...
The `utf8mb4_unicode_ci` collation makes identifier comparison case-insensitive and accent-insensitive, and MySQL ignores trailing spaces when comparing these `VARCHAR` values. For example, `Café`, `cafe` and `CAFÉ   ` identify the same value. Redis keys preserve the exact submitted strings, so equivalent spellings may create separate cache entries, but every cache miss resolves through MySQL and receives the same persisted `userID`.

The composite unique key on `(id1, id2)` is the concurrency boundary. Each request attempts an insert using `ON DUPLICATE KEY UPDATE`; when requests race, MySQL retains one row. Every request then selects that row, so all callers receive the persisted UUID rather than relying on the UUID generated by that particular request. The live integration suite verifies this with 24 concurrent requests.
...
| `npm run format` | Format application, unit and integration TypeScript using the repository's single-quote style. |
...
The validation suite in `test/validation.test.ts` exercises the same validation-pipe factory used by application startup. Its 21 cases check valid inputs, length boundaries, missing values, invalid types, empty, whitespace-only or oversized identifiers, and unexpected properties. Rejected inputs must produce a 400 exception with a message identifying the invalid field.
...
The 86 default tests require no `.env`, HTTP server, MySQL or Redis. Live checks are separate so these fast tests can run without external services.
...
Verified against Redis 7.4.11: all live checks passed, including recovery of both an established connection and a client started during the outage.
...
- MySQL's documented case, accent and trailing-space identifier comparison.
...
Open the MySQL monitor in the Compose service:

```bash
docker compose exec mysql sh -c 'exec mysql -u"$MYSQL_USER" -p "$MYSQL_DATABASE"'
```

Enter the application database password from your local `.env` when prompted. This command uses the service name, so it is independent of Compose's generated container name.
